...
def get_tracker(frames, labels_dir):
    '''
    Extracts the x,y coordinates of the tracker from the frames of the video
    '''
    x = []
    y = []
    ...
    for frame_num in all_frames:
        ...
        if frame_num != int(frame[ind[0][0] + 1:ind[0][1] - 4]):
            ...
            with open(os.path.join(labels_dir, frame), encoding="utf-8") as f:
                lines = f.readlines()
                # TODO: deal with multiple detections per frame
                line = lines[0]
                line = line.split()
